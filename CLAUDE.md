...
基于 RAG 的本地知识库问答系统。文档加载→索引构建→两阶段检索→LLM生成回答。Vue 3 + Tailwind 前端，FastAPI 后端。
...
src/
├── config.py        # 共享配置：路径常量
├── loader.py        # 文档解析：.txt/.md/.docx/.pdf（自动编码检测、表格提取）
...
├── generator.py      # LLM生成：调用OpenAI兼容API基于检索结果生成回答
├── api.py            # FastAPI REST API端点

frontend/
├── src/views/        # Vue页面（ChatView, FilesView, SettingsView, StatsView, HistoryView）
├── src/api.js        # 前端API客户端
├── src/App.vue       # 主布局+导航
...
- **LLM配置持久化**：保存在data/llm_config.json，不在代码中硬编码
- **Markdown解析**：用markdown+BeautifulSoup转纯文本，去掉标记保留结构

## 依赖版本约束

- `transformers>=4.45,<5`：FlagEmbedding不兼容transformers 5.x
...
RTX 3060 Laptop 6GB显存，同时加载embedding(~2.2GB)+reranker(~1.2GB)=~3.4GB，够用。CPU也可运行。
